defmodule Miner.ScraperTest do
  ...
  test "can process all items in queue and see results" do
  	assert Miner.Scraper.process_queue == 3
    ...
    assert Miner.TaskQueue.getAtIndex(Miner.TaskQueue, 2).status_code == 200
    assert Miner.TaskQueue.getAtIndex(Miner.TaskQueue, 2).results == ["Pattern matching.", "The match operator", "The pin operator"]
  end
end