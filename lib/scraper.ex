defmodule Miner.Scraper do
  ...
  defp handle_result(res, index, task) do
    try do
      ...
      results =
        Enum.map(task.xpq, fn query -> results ++ Miner.XPQ.text(res.body, query.selector) end)

      Miner.TaskQueue.update(Miner.TaskQueue, index, %{
        url: task.url,
        xpq: task.xpq,
        results: results,
  ...
  defp task_reducer() do
    Enum.reduce(Miner.TaskQueue.get(Miner.TaskQueue) |> Tuple.to_list(), 0, fn task, index ->
      spawn_task(task) |> handle_result(index, task)
    end)
  end
end
